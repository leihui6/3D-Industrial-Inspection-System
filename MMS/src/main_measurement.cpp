...
int main(int argc, char *argv[])
{
	...
	// load measure pairs
	std::multimap<std::string, std::string> measurement_pairs_map, reference_map;
	read_file_as_map(measurement_pairs, measurement_pairs_map, reference_map);
	...
	load_point_cloud_txt(reading_data_name, reading_data, false);
	...
	//save_points(reading_data,"tmp.txt");
	// begin to measurement including dimensional inspection and defect analysis

	// 1. dimensional inspection
	cloud_measurement cm(reading_data);
	std::vector<measurement_content> mc_vec;
	cm.measure(measurement_pairs_map, marked_points_map, reference_map, mc_vec);
	export_measured_data(measurement_pairs_map, mc_vec, output_folder + "/measurement_results.txt");
